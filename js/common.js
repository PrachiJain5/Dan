...
// Event tracking codes - jQuery must be installed
...
$(function() {
	...
	$('a[href$="pdf"]').each(function() { // ADD GA tracking code to links
		$(this).attr({
			'onclick': "ga('send', 'event', 'PDF', 'Clicked', jQuery(this).attr('href'), {'nonInteraction': 1});",
			'target': '_blank'
		});
	});
	...
	$('a[href$=".doc"], a[href$=".docx"], a[href$=".xls"], a[href$=".xlsx"], a[href$=".ppt"], a[href$=".pptx"]').each(function() {
		$(this).attr('onclick', "ga('send', 'event', 'Media', 'Clicked', jQuery(this).attr('href'), {'nonInteraction': 1});"); // ADD GA tracking code to links
	});

	//FIND links that start with http
	$('a[href^="http"]').each(function() { // ADD GA tracking code to links
		$(this).attr({
			'onclick': "ga('send', 'event', 'Outbound', 'Clicked', jQuery(this).attr('href'), {'nonInteraction': 1});",
			'target': '_blank'
		});
	});

	//FIND weblinks if used on Joomla site
	$('a[href*="weblinks"]').each(function() { // ADD GA tracking code to links
		$(this).attr('onclick', "ga('send', 'event', 'Outbound', 'Weblink Clicked', jQuery(this).attr('href'), {'nonInteraction': 1});");
    });
...
//         // ADD GA tracking code to links
//         //$(this).attr('onclick', "ga('send', 'event', 'search', 'keyword', enteredvalue, clickedText, jQuery(this).attr('href'), {'nonInteraction': 1});");
...
$("#tipue_drop_content").on('click', function (event) {
    // alert("tipue_drop_content");
    var clickedText = event.target.innerHTML;
    console.log(clickedText);
    console.log($("#tipue_search_input").val());

    //latest ga string:
    ga('send', 'event', $("#tipue_search_input").val(), clickedText, "testvalue4", {    
        'nonInteraction': 1
    });

     //original:
    //ga('send', 'event', 'search', 'keyword', $("#tipue_search_input").val(), clickedText);
})
